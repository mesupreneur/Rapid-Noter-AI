...
function SideBar() {
  ...
  return (
    <div className="shadow-md h-screen p-7">
      ...
      <div className="mt-10">
        <UploadPdfDialog
        isMaxFile={fileList?.length >= (GetUserInfo?.upgrade ? 30 : 3)}
        >
      ...
      {!GetUserInfo?.upgrade && (
        <div className="absolute bottom-24 w-[78%]">
          <Progress value={(fileList?.length / 3) * 100} />
          <p className="text-sm mt-1">
            {fileList?.length} out of 3 PDFs Uploaded
          </p>
      ...
      {GetUserInfo?.upgrade && (
        <div className="absolute bottom-24 w-[78%]">
          ...
          <p className="text-sm mt-1">
            {fileList?.length} out of 30 PDFs Uploaded
          </p>
        </div>
      )}
    </div>
